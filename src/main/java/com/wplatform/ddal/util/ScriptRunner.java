...
public class ScriptRunner {
    ...
    private void executeFullScript(Reader reader) {
        ...
        try {
            ...
            throw DbException.throwInternalError(message);
        }
    }
    ...
    private void setAutoCommit() {
        try {
            ...
            throw DbException.throwInternalError("Could not set AutoCommit to " + autoCommit + ". Cause: " + t);
        }
    }
    ...
    private void commitConnection() {
        try {
            ...
            throw DbException.throwInternalError("Could not commit transaction. Cause: " + t);
        }
    }
    ...
    private void checkForMissingLineTerminator(StringBuilder command) {
        if (command != null && command.toString().trim().length() > 0) {
            throw DbException.throwInternalError("Line missing end-of-line terminator (" + delimiter + ") => " + command);
        }
    }
